...
from test import Accurate,Correct,Pare,Performance
# from tes import *  和test相差2s/图
...
if __name__ == '__main__':
    ...
    parser.add_argument('--result-path', type=str, default='results_.csv')
    ...
    with open(args.result_path, 'w', newline='') as csvfile:
        # 定义CSV列名
        fieldnames = ['filename','H','pitch','accurate', 'correct', 'performance']
        ...
        for k, filename in enumerate(filenames):
            ...
            compare_filename = f"Compare_{os.path.splitext(os.path.basename(filename))[0]}" + '.png'
            compare_filepath = os.path.join(item_output_folder_path,compare_filename)
            #这里我们对方法的实际效果进行检验
            #1、精确性：与划定safe区域的交并比
            #2、正确性：不能unsafe区域
            #3、质量因子：
            # Test(jsonname, rotation_image, result_mask, compare_filepath)
